...
ODE_Res ode23t(const ODE_Func& F, const std::vector<double>& tspan, const std::vector<double>& Init_Value, const ODE_Set& Options)
{
	if (tspan.size() < 2)
	{
		throw std::invalid_argument("ode23t: tspan must contain at least two elements.");
	}
	...
	for (size_t i = 1; i < tspan.size(); ++i)
	{
		...
		{
			throw std::invalid_argument("ode23t: tspan must be strictly increasing");
		}

		if (!increasing && tspan[i] > tspan[i - 1])
		{
			throw std::invalid_argument("ode23t: tspan must be strictly decreasing");
		}
	}
	...
	if (n == 0)
	{
		throw std::invalid_argument("ode23t: initial state is empty");
	}
	...
	const size_t max_steps_per_jac = 10;
	...
	while ((direction > 0.0 && t < tf) || (direction < 0.0 && t > tf))
	{
		if (++iter > max_iter)
		{
			throw std::runtime_error("ode23t: Maximum iterations exceeded.");
		}
		...
		while (!step_accepted)
		{
			if (std::abs(h) < h_min)
			{
				throw std::runtime_error("ode23t: step size too small; failed to converge to acceptable tolerances");
			}
			...
			if (needs_fac)
			{
				...
				for (size_t c = 0; c < n; ++c)
				{
					...
					{
						W(static_cast<int>(r), static_cast<int>(c)) -= h05 * J(static_cast<int>(r), static_cast<int>(c));
					}
				}
			...
			for (size_t i = 0; i < n; ++i)
			{
				y_pred[i] = y[i] + h * f_current[i];
			}

			y_next = y_pred;
			bool converged = false;

			for (size_t newt = 0; newt < max_newton_iter; ++newt)
			{
				f_next = F(t + h, y_next);
				if (Options.Stats)
				{
				...
				for (size_t i = 0; i < n; ++i)
				{
					rhs[i] = y[i] + h05 * (f_current[i] + f_next[i]) - y_next[i];
				}

				vec rhs_vec(rhs);
				vec delta_vec = lu_solve(W, pivots, rhs_vec);
				for (size_t i = 0; i < n; ++i)
				{
					delta[i] = delta_vec[i];
					y_next[i] += delta[i];
				...
				else
				{
					if (Options.Refine > 1)
					{
						for (int j = 1; j < Options.Refine; ++j)
						{
							...
							res.Result.push_back(hermite_dense(theta, h, y_prev, y,
								f_prev, f_current));
						}
					}
